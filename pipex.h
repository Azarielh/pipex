...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/25 01:43:38 by jlacaze-          #+#    #+#             */
/*   Updated: 2025/06/25 02:06:56 by jlacaze-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief The t_pipex struc is meant to make your life easier while coding
 * 			the so called function. You can store here every args separatly
 *			depending on what it is (infile, commands and outfile).
 * item :
...
/**
 * @brief This function will do its own check before giving birth through
 * 			the fork function. Dealing with dup2 to redirect properly
 *			the fd you're working with.
 * 
 * @param cmd This have to be a string corresponding to a tty command.
 * @param env You have to pass your env to this function.
...
/**
 * @brief Perfect when you're looking for some specific
 * 			in an environment passed in argument.
 *
 * @param env The environment you want to look in.
 * 				It's usually yours for now but who knows later on.
 * 
 * @param key What it is you're looking for in that peculiar env?
 * 
 * @return It will get you the string you want for the work you're on.
 *			A path? A Cpu intel? Anything related to the key you write upthere.
 */
char	**getent_env(char **env);
/**
 * @brief This function will free the content of the double pointer passed
 *			in argument.
 * 
